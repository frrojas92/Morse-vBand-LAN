...
function downloadLog(channel, target = '', callsign = 'room', format = 'csv') {
  socket.emit('logs:get', { channel, target }, answer => {
    ...
    const content = format === 'csv'
      ...
      : answer.entries.map(entry => `[${entry.timestamp}] ${entry.direction} ${entry.callsign} | ${entry.morse} | ${entry.text} | ${entry.wpm} WPM | ${entry.mode} | ${entry.toneFrequency} Hz ${entry.toneWaveform} | ${entry.keyDurationMs} ms`).join('\n');
...
function render({ channels }) {
  ...
  for (const room of channels) {
    ...
    for (const op of room.operators) { const row = document.createElement('div'); const last = op.lastTransmitAt ? new Date(op.lastTransmitAt).toLocaleTimeString() : 'nunca'; row.innerHTML = `<span><b>${op.callsign}</b><small>${op.keyDowns} pulsaciones · última ${last}</small></span>`; if (room.policy.decodeText || room.policy.decodeCode) { const decoded = document.createElement('div'); decoded.className = 'instructor-decode'; if (room.policy.decodeText) { const text = document.createElement('strong'); text.textContent = op.text || '…'; decoded.append(text); } if (room.policy.decodeCode) { const code = document.createElement('code'); code.textContent = op.code || '…'; decoded.append(code); } row.append(decoded); } row.append(button(`Decodificador ${op.decoderEnabled ? 'ACTIVO' : 'INACTIVO'}`, () => action(room.channel, 'studentDecoder', !op.decoderEnabled, op.id)), button('TXT', () => downloadLog(room.channel, op.id, op.callsign, 'txt')), button('CSV', () => downloadLog(room.channel, op.id, op.callsign)), button(op.muted ? 'Activar audio' : 'Silenciar', () => action(room.channel, 'mute', !op.muted, op.id)), button(op.reserved ? 'Liberar TX' : 'Reservar TX', () => action(room.channel, 'reserve', null, op.reserved ? '' : op.id)), button('Desconectar', () => action(room.channel, 'disconnect', null, op.id), true)); list.append(row); }
    card.append(list); root.append(card);
  }
  if (!channels.length) root.textContent = 'No hay canales. Cree uno arriba.';
}
$('#loginForm').onsubmit = event => { event.preventDefault(); socket.emit('instructor:login', { pin: $('#pin').value, callsign: $('#instructorCallsign').value }, answer => { if (!answer?.ok) return status(answer?.reason, true); authenticated = true; $('#loginPanel').hidden = true; $('#desk').hidden = false; render(answer); }); };
...
socket.on('disconnect', () => { authenticated = false; $('#desk').hidden = true; $('#loginPanel').hidden = false; status('Desconectado. Ingrese nuevamente después de la reconexión.', true); });
